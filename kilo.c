...
void editorFind() {
  ...
  // else {
  //    editorStatusMessage("Pattern not found: %s", query);
  //  }
  if (query) {
      free(query);
  } else {
  ...
  if (fd != -1) {
    if (ftruncate(fd, len) != -1) {
      if (write(fd, buf, len) == len) {
        ...
        free(buf);
        E.dirty = 0;
        editorStatusMessage("Wrote %d bytes to %s", len, E.filename); 
        return;
      }
    }
    close(fd);
  ...
  switch(c) {
    ...
    case CTRL_KEY('q'):
      if (E.dirty && quit_times > 0) {
        editorStatusMessage("WARNING! The file has unsaved changes. Press Ctrl-Q %d more times to quit.", quit_times);
        quit_times--;
        return;
      } 
      write(STDOUT_FILENO, "\x1b[2J", 4);
